...
local function testKurt ()
	...
	assert(linear.kurt(x, 1) ~= linear.kurt(x, 1))
	x = linear.tolinear({ 1, 2 })
	...
	assert(math.abs(linear.kurt(x, 1) - (-1.2)) < EPSILON)
	x = linear.tolinear({ 1, 2, 3, 5 })
	assert(math.abs(linear.kurt(x) - (-1.154286)) < EPSILON)
	assert(math.abs(linear.kurt(x, 1) - (0.342857)) < EPSILON)
end
