...
local function testRanks ()
	local ranks = linear.ranks(1)
	assert(#ranks == 0)
	ranks = linear.ranks(1, "z")
	assert(#ranks == 1)
	assert(ranks[1] == 0)
	ranks = linear.ranks(1, "q")
	assert(#ranks == 1)
	assert(ranks[1] == 1)
	ranks = linear.ranks(1, "zq")
	assert(#ranks == 2)
	assert(ranks[1] == 0)
	assert(ranks[2] == 1)
	ranks = linear.ranks(2)
	assert(#ranks == 1)
	assert(ranks[1] == 0.5)
	ranks = linear.ranks(2, "z")
	assert(#ranks == 2)
	assert(ranks[1] == 0)
	assert(ranks[2] == 0.5)
	ranks = linear.ranks(2, "q")
	assert(#ranks == 2)
	assert(ranks[1] == 0.5)
	assert(ranks[2] == 1)
	ranks = linear.ranks(2, "zq")
	assert(#ranks == 3)
	assert(ranks[1] == 0)
	assert(ranks[2] == 0.5)
	assert(ranks[3] == 1)
	ranks = linear.ranks(4)
	assert(#ranks == 3)
	assert(ranks[1] == 0.25)
	assert(ranks[2] == 0.5)
	assert(ranks[3] == 0.75)
...
local function testQuantile ()
	...
	assert(linear.quantile(x, 0) == 1)
	assert(linear.quantile(x, 0.5) == 2)
	assert(linear.quantile(x, 1) == 3)
	assert(linear.quantile(x, 0.25) == 1.5)
	local q = linear.quantile(x, { 0, 0.25, 0.5, 1 })
	assert(#q == 4)
	assert(q[1] == 1)
	assert(q[2] == 1.5)
	assert(q[3] == 2)
...
local function testRank ()
	local x = linear.tolinear({ 1, 3, 2 })
	assert(linear.rank(x, 1) == 0)
	assert(linear.rank(x, 2) == 0.5)
	assert(linear.rank(x, 3) == 1)
	assert(linear.rank(x, 1.5) == 0.25)
	local r = linear.rank(x, { 1, 2, 3, 1.5 })
	assert(#r == 4)
	assert(r[1] == 0)
	assert(r[2] == 0.5)
	assert(r[3] == 1)
