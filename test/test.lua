...
local function testSkew ()
	...
	assert(linear.skew(x, "sample") ~= linear.skew(x, "sample"))
	x = linear.tolinear({ 1, 2 })
	assert(linear.skew(x) == 0)
	assert(linear.skew(x, "sample") ~= linear.skew(x, "sample"))
	local x = linear.tolinear({ 1, 2, 3 })
	assert(linear.skew(x) == 0)
	assert(linear.skew(x, "sample") == 0)
	x = linear.tolinear({ 1, 2, 4 })
	assert(math.abs(linear.skew(x) - 0.381802) < EPSILON)
	assert(math.abs(linear.skew(x, "sample") - 0.935220) < EPSILON)
end
...
local function testKurt ()
	...
	assert(linear.kurt(x, "sample") ~= linear.kurt(x, "sample"))
	x = linear.tolinear({ 1, 2 })
	...
	assert(math.abs(linear.kurt(x, "sample") - (-1.2)) < EPSILON)
