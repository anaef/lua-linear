...
value, formally $\mathop{\mathrm{argmax}}_i \ \| x_i \|$.


#### `linear.sum (vector|matrix x [, vector y [, transpose]])`

Returns the sum of the elements of a vector, formally $\sum_i x_i$, or sets a
vector to the sum of each major order vector of a matrix, formally
$y_i \leftarrow \sum_j x_{i,j}$. The argument transpose is one of `notrans`, `trans`, and
defaults to `notrans`. If set to `trans`, the vector is set to the sum of each
minor order vector of the matrix, formally $y_j \leftarrow \sum_i x_{i,j}$.


#### `linear.swap (vector|matrix x, vector|matrix y [, transpose])`

Swaps the elements of two vectors or matrices, formally $x \leftrightarrow y$. The function
...
$y \leftarrow x$. The function can be invoked with a vector and a matrix to copy the
...
$y \leftarrow \alpha x + y$. The function can be invoked with a vector and a matrix to
...
Scales a vecctor or matrix, formally $x \leftarrow \alpha x$. The argument $\alpha$ defaults
to $1.0$.


#### `linear.set (vector|matrix x [, alpha])`

Sets the elements of a vector or matrix to a constant, formally $x \leftarrow \alpha$.
...
formally $x \leftarrow u \in \mathcal{U}(0, 1 - \epsilon)$.


#### `linear.normal (vector|matrix x)`

Sets the elements of a vector or matrix to normally distributed random values,
formally $x \leftarrow n \in \mathcal{N}(0, 1)$.


#### `linear.inc (vector|matrix x [, alpha])`

Increments the elements of a vector or matrix, formally $x \leftarrow x + \alpha$. The
...
$y \leftarrow x^\alpha y$.  The function can be invoked with a vector and a matrix to
...
Raises the elements of a vector or matrix to a power, formally $x \leftarrow x^\alpha$.
The argument $\alpha$ defaults to $1.0$.
...
$x \leftarrow \mathop{\mathrm{sgn}} \ x$, or returns the sign of a number,
formally $\mathop{\mathrm{sgn}} \ x$. The sign is $1$ if $x > 0$, $-1$ if $x < 0$,
and $x$ otherwise.
...
formally $x \leftarrow \| x \|$, or returns the absolute value of a number, formally
$\| x \|$.
...
$x \leftarrow e^x$, or returns the exponent of a number, formally $e^x$.


#### `linear.log (number|vector|matrix x)`

Applies the natural logarithm function to the elements of a vector or matrix,
formally $x \leftarrow \log x$, or returns the natural logarithm of a number, formally
$\log x$.
...
$x \leftarrow \frac{1}{1 + e^{-x}}$, or returns the logistic function of a number,
formally $\frac{1}{1 + e^{-x}}$.


#### `linear.tanh (number|vector|matrix x)`

Applies the hyperbolic tangent function to the elements of a vector or matrix,
formally $x \leftarrow \tanh x$, or returns the hyperbolic tangent of a number, formally
$\tanh x$\.
...
$x \leftarrow \log (1 + e^x)$, or returns the softplus function of a number, formally
$\log (1 + e^x)$.
...
the elements of a vector or matrix, formally $x \leftarrow \max_{0, x}$, or returns the
recitifier function of a number, formally $\max_{0, x}$.
...
$x \leftarrow f(x)$, or returns the function value of a number, formally $f(x)$.
...
$y \leftarrow \alpha A x + \beta y$. The arguments $\alpha$ and $\beta$ default to $1.0$ and
...
$A \leftarrow \alpha x y^T + A$. The argument $\alpha$ defaults to $1.0$.
...
$C \leftarrow \alpha A B + \beta C$. The arguments $\alpha$ and $\beta$ default to $1.0$ and
...
systems, the function solves the least squares problems $min \lVert b - A x \rVert_2$,
...
Inverts a matrix, formally $A \leftarrow A^{-1}$. Returns 0 if the calculation was
...
#### `linear.cov (matrix A, matrix B, ddof)`

Caclulates the pairwise covariances of the column vectors of A with the specified delta
degrees of freedom, and places the covariances into B, formally $B_{i, j} \leftarrow
\frac{\sum_y (A_{i,y} - \bar{A_i}) (A_{j,y} - \bar{A_j})}{N - \textrm{ddof}}$ for a
columm major matrix.


#### `linear.corr (matrix A, matrix B)`

Caclulates the pairwise Pearson correlations of the column vectors of A and places them
into B, formally $B_{i, j} \leftarrow \frac{\sum_y (A_{i,y} - \bar{A_i}) (A_{j,y} - \bar{A_j})}
{\sqrt{\sum_y (A_{i,y} - \bar{A_i})^2 \times \sum_y (A_{j,y} - \bar{A_j})^2}}$ for a
column major matrix.
